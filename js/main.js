  ...
  for (var i = 0; i < pins.length; i++) {
    (function (pin, card) {
      pin.addEventListener('click', function () {
        ...
        openedCard = card;
        openedCard.querySelector('button.popup__close').addEventListener('click', closeOpenedCardHandler);
        document.addEventListener('keydown', onEscCloseWindow);
      });
    })(pins[i], cards[i]);
  }
};
mainPin.addEventListener('mousedown', onMouseDownActivate);
mainPin.addEventListener('keydown', onKeyDownActivate);
