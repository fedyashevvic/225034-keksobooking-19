...
var generateBookingData = function () {
  ...
  for (var i = 0; i < numberOfData; i++) {
    ...
    var y = Math.round(Math.random() * (yPositions[1] - yPositions[0]) + yPositions[0]);
    var currentData = {
      author: {
...
var renderPinElement = function (data) {
  ...
  var positioning = 'left: ' + (data.location.x - pinWidth) + 'px; top: ' + (data.location.y - pinHeight) + 'px;';
